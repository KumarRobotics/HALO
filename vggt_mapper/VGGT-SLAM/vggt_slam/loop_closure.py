...
class ImageRetrieval:
    ...
    def get_all_submap_embeddings(self, submap):
        ...
        frames = submap.get_all_frames().cuda() # put back on gpu
        return self.get_batch_descriptors(frames)

    def find_loop_closures(self, map, submap, candidate_ids, max_similarity_thres = 0.80, max_loop_closures = 0): # TODO make these paramaters in a config file
        matches_queue = LoopMatchQueue(max_size=max_loop_closures)
        query_id = 0
        for query_vector in submap.get_all_retrieval_vectors():
            best_score, best_submap_id, best_frame_id = map.retrieve_best_score_frame(query_vector, submap.get_id(), candidate_ids, ignore_last_submap=True)
