...
from .base import LangSpatialGlobalImageEncoder
...
if __name__ == "__main__":
  # Example usage
  import torchvision
  rgb_img = torchvision.io.read_image("test.jpg")
  rgb_img = rgb_img.float() / 255
  rgb_img = torch.nn.functional.interpolate(
    rgb_img.unsqueeze(0), size=(512, 512))
  ...
  enc = NARadioEncoder(model_version="radio_v2.5-b", lang_model="siglip",
                       input_resolution=[512, 512])
  rgb_img = rgb_img.to(enc.device)  # Ensure image is on the same device as the model
  
  feat_map = enc.encode_image_to_feat_map(rgb_img)
  lang_aligned_feat_map = enc.align_spatial_features_with_language(feat_map)

  text_features = enc.encode_labels(labels)
  lang_aligned_feat_map = lang_aligned_feat_map.squeeze()
  h, w = lang_aligned_feat_map.shape[1:3]
  lang_aligned_feat_map = lang_aligned_feat_map.permute(1, 2, 0).reshape(h*w, 1152)
  print(lang_aligned_feat_map.shape)
  print(text_features.shape)

  from utils import compute_cos_sim
  r = compute_cos_sim(text_features, lang_aligned_feat_map, softmax=False)
  r = r.reshape(h, w, -1).detach().cpu().numpy()

  r = r[:, :, 0]

  # Visualize r as heatmap and overlay on original image
  import matplotlib.pyplot as plt
  import numpy as np
  import torch.nn.functional as F

  # Resize heatmap to original image size
  heatmap = torch.tensor(r)
  heatmap = heatmap.unsqueeze(0).unsqueeze(0)  # shape: 1x1xhxw
  heatmap_resized = F.interpolate(heatmap, size=(rgb_img.shape[2], rgb_img.shape[3]), mode="bilinear", align_corners=False)
  ...
  img_disp = rgb_img.squeeze().permute(1, 2, 0).cpu().numpy()
